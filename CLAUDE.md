...
and next-delivery timestamps from the DHL, PostNL, DPD, GLS, Dragonfly, Trunkrs and Cainiao
integrations
...
- **Canonical `ParcelStatus` enum** in `const.py` — mirrors the enum
  the per-carrier integrations (DHL, DPD, PostNL, GLS, Dragonfly, Trunkrs, Cainiao) publish on the
