...
A Home Assistant custom integration that rolls up parcel counts, next-delivery timestamps, and parcel-event notifications from the DHL, PostNL, DPD, GLS, Dragonfly and Trunkrs integrations into a single set of sensors and a single unified event stream.
...
- A single dashboard card that shows how many parcels you expect today across DHL, PostNL, DPD, GLS, Dragonfly and Trunkrs without juggling per-carrier sensors.
...
| Key | Type | Meaning |
|---|---|---|
| `carrier` | string | `"DHL"`, `"PostNL"`, `"DPD"`, `"GLS"`, `"Dragonfly"` or `"Trunkrs"` |
...
| `parcel_aggregator_parcel_status_changed` | A known parcel's `status` value changes, except the final hop to delivered | Same payload plus `old_status` and `new_status` |
| `parcel_aggregator_parcel_delivered` | An incoming parcel is delivered | The full parcel dict |
...
This is an independent, community-built project with no affiliation, endorsement, or connection to DHL, PostNL, DPD, GLS, Dragonfly Shipping, Trunkrs, or any of their subsidiaries.
